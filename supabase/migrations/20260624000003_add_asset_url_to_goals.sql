-- ============================================
-- Migration: Add asset_url to goals and update complete_task RPC
-- ============================================

-- 1. Add the column if it doesn't exist
ALTER TABLE public.goals ADD COLUMN IF NOT EXISTS asset_url text;
...
CREATE OR REPLACE FUNCTION public.complete_task(
  ...
  p_mature_asset_url text default null
)
RETURNS jsonb
LANGUAGE plpgsql
...
DECLARE
  ...
  v_new_plant_state public.plant_state;
  v_new_asset_url text;
BEGIN
  -- Get current user
  v_user_id := auth.uid();
  ...
  -- Determine state and asset
  if v_new_rate >= 1.0 then
    v_new_plant_state := 'MATURE_STATE'::public.plant_state;
    v_new_asset_url := p_mature_asset_url;
  else
    v_new_plant_state := 'GROWING'::public.plant_state;
    v_new_asset_url := v_goal.asset_url; -- keep existing if any
  end if;
  ...
  set
    ...
    plant_state = v_new_plant_state,
    asset_url = v_new_asset_url
  where id = p_goal_id;
